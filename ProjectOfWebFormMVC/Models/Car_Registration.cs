...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class Car_RegistrationContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Car_Registration> GetCars()
        {
            List<Car_Registration> cars = new List<Car_Registration>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg", conn);
            conn.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            while(dr.Read())
            {
                Car_Registration car = new Car_Registration();
        ...
        public bool AddCars(Car_Registration c)
        {
            ...
            {
                SqlConnection conn = new SqlConnection(cs);
                SqlCommand cmd = new SqlCommand("insert into CAR_Reg values(@COMPONY_NAME,@CAR_NAME,@CAR_NO,@CAR_MODEL,@OWNER_NAME,@YEAR,@CITY,@CAR_PRICE,@DATE)", conn);
                cmd.Parameters.AddWithValue("@COMPONY_NAME", c.Compony_Name);
        ...
        public bool UpdateCars(Car_Registration c)
        {
            ...
            {
                SqlConnection conn = new SqlConnection(cs);
                SqlCommand cmd = new SqlCommand("update CAR_Reg set COMPONY_NAME=@COMPONY_NAME,CAR_NAME=@CAR_NAME,CAR_NO=@CAR_NO,CAR_MODEL=@CAR_MODEL,OWNER_NAME=@OWNER_NAME,YEAR=@YEAR,CITY=@CITY,CAR_PRICE=@CAR_PRICE,DATE=@DATE where CAR_ID=@CAR_ID ", conn);
