...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class JeepContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Jeep> GetCars()
        {
            List<Jeep> cars = new List<Jeep>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='JEEP'", conn);
            conn.Open();
