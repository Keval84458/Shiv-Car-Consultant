...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class FordContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Ford> GetCars()
        {
            List<Ford> cars = new List<Ford>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='FORD'", conn);
            conn.Open();
