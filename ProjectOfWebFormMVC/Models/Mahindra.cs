...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class MahindraContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Mahindra> GetCars()
        {
            List<Mahindra> cars = new List<Mahindra>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='MAHINDRA'", conn);
            conn.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                Mahindra car = new Mahindra();
        ...
        public bool UpdateCars(Mahindra c)
        {
