...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class TataContext
    {

        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Tata> GetCars()
        {
            List<Tata> cars = new List<Tata>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='TATA'", conn);
            conn.Open();
