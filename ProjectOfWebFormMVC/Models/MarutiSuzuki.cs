...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class MarutiSuzukiContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<MarutiSuzuki> GetCars()
        {
            List<MarutiSuzuki> cars = new List<MarutiSuzuki>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='MARUTI SUZUKI'", conn);
            conn.Open();
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                MarutiSuzuki car = new MarutiSuzuki();
        ...
        public bool UpdateCars(MarutiSuzuki c)
        {
