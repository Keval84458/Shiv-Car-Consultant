...
namespace ProjectOfWebFormMVC.Models
{
    ...
    public class KiaContext
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;

        public List<Kia> GetCars()
        {
            List<Kia> cars = new List<Kia>();
            SqlConnection conn = new SqlConnection(cs);
            SqlCommand cmd = new SqlCommand("select *from CAR_Reg where COMPONY_NAME='KIA'", conn);
            conn.Open();
